...
set "OUTDIR=%ROOT%\bin"
set "OBJDIR=%LOCALAPPDATA%\FelWorks\BLOCKWELT\obj"
if not exist "%LOCALAPPDATA%\FelWorks" mkdir "%LOCALAPPDATA%\FelWorks"
if not exist "%LOCALAPPDATA%\FelWorks\BLOCKWELT" mkdir "%LOCALAPPDATA%\FelWorks\BLOCKWELT"

echo.
echo  ==================================================
echo   Blockwelt by FelWorks - Build
echo  ==================================================
echo.
...
if /i "%~1"=="clean" (
  ...
  del /q "%OUTDIR%\BLOCKWELT.exe" >nul 2>&1
)
...
set "LIBS=opengl32.lib user32.lib gdi32.lib shell32.lib winmm.lib ws2_32.lib"
...
pushd "%OBJDIR%"
cl %CFLAGS% %INCLUDES% %OPTFLAGS% /MP !SRCLIST! /Fe:"%OUTDIR%\BLOCKWELT.exe" /Fd:"%OBJDIR%\BLOCKWELT.pdb" %LIBS% %LDFLAGS%
set BUILDERR=%errorlevel%
if not "%BUILDERR%"=="0" (
  echo.
  echo  Wiederhole Build ohne Parallelisierung...
  echo.
  cl %CFLAGS% %INCLUDES% %OPTFLAGS% !SRCLIST! /Fe:"%OUTDIR%\BLOCKWELT.exe" /Fd:"%OBJDIR%\BLOCKWELT.pdb" %LIBS% %LDFLAGS%
  set BUILDERR=!errorlevel!
)
popd
...
echo  [4/4] Fertig: %OUTDIR%\BLOCKWELT.exe
echo.
echo  ==================================================
