...
set "ROOT=%ROOT:~0,-1%"
set "OUTDIR=%ROOT%\bin"
set "OBJDIR=%LOCALAPPDATA%\FelWorks\STARFALL\obj"
if not exist "%LOCALAPPDATA%\FelWorks" mkdir "%LOCALAPPDATA%\FelWorks"
if not exist "%LOCALAPPDATA%\FelWorks\STARFALL" mkdir "%LOCALAPPDATA%\FelWorks\STARFALL"

echo.
echo  ==================================================
echo   STARFALL: BEYOND THE VOID by FelWorks - Build
echo  ==================================================
echo.
...
if /i "%~1"=="clean" (
  ...
  del /q "%OUTDIR%\STARFALL.exe" >nul 2>&1
)
...
set "LIBS=opengl32.lib user32.lib gdi32.lib shell32.lib ole32.lib winmm.lib ws2_32.lib"
...
pushd "%OBJDIR%"
cl %CFLAGS% %OPTFLAGS% /MP !SRCLIST! /Fe:"%OUTDIR%\STARFALL.exe" /Fd:"%OBJDIR%\STARFALL.pdb" %LIBS% %LDFLAGS%
set BUILDERR=%errorlevel%
if not "%BUILDERR%"=="0" (
  echo.
  echo  Wiederhole Build ohne Parallelisierung...
  echo.
  cl %CFLAGS% %OPTFLAGS% !SRCLIST! /Fe:"%OUTDIR%\STARFALL.exe" /Fd:"%OBJDIR%\STARFALL.pdb" %LIBS% %LDFLAGS%
  set BUILDERR=!errorlevel!
)
popd
...
echo  [4/4] Fertig: %OUTDIR%\STARFALL.exe
echo.
echo  ==================================================
