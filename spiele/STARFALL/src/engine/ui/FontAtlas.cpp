...
const u8 kFontData[kGlyphCount][kGlyphColumns] = {
...
const u8* glyphColumns(u32 codepoint) {
    if (!glyphExists(codepoint)) return kFontData[0];
    return kFontData[codepoint - kGlyphFirst];
}

bool glyphExists(u32 codepoint) {
    return codepoint >= kGlyphFirst && codepoint < kGlyphFirst + kGlyphCount;
}
